...
Note: Throughout the project I will be making references and comparisons to an existing rendering system from a book called 'Physically Based Rendering Techniques'<sup>[[1](#1)]</sup> for which I will use the abbreviation PBRT (which is also the name given to the official renderer created based on the book).
...
The manual testing side was mainly validation that the image output seemed coherent enough as to what a physical environment would represent and from this I was able to trace down a lot of bugs by noticing that the colours were not quite right or the reflections were not in the correct angle. Every now and then, especially when adding a new feature, after I had validated it by eye I would render a similar scene using the PBRT ray tracer with the advantage that I could use the same file format (since I had designed my application this way) which allowed me to compare the output images and see if it was correct. This was especially useful when testing out refraction in the glass material, this was because I couldn't intuitively pick out which way the light should refract and it was only upon comparison that I realised that the ray tracer would refracting light in the wrong direction.
