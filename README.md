...
Next, it is the role of the sampler to pick which points on the screen to rays through. The most basic implementation would select a single light ray through the centre of each pixel in the image. However, this doesn't always produce the best results for more complex images where multiple rays will need to be fired for each pixel and in different locations. Once a location has been chosen, the camera actually generates the ray by taking into account parameters such as depth of field which help to simulate realistic camera output.
...
The first type of camera is called an Orthographic camera, this camera generates rays by 
