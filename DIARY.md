...
##### 20/04/17
Since the last diary entry, I have added a lot more material definitions and also included a new spot light source that I have used for some interesting effects that looks more realistic than the existing point light implementation. I spent a whole week fixing a buggy implementation is both the sphere and triangle intersection routines which were causing some issues in the reflectance sampling code which was quite frustrating at the time but I'm convinced the code is now correct since I ended up reviewing a lot of code before I found the source the bug. As a result, I now aim to focusing more on creating more new code and wrapping up the programming side. 
